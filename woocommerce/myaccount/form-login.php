...
<!-- Login content (layout/background on main#primary) -->
<div class="relative w-full">
	...
	<div class="relative flex-1 flex flex-col items-center max-w-2xl mx-auto w-full px-4 pb-8 lg:pb-10">
       ...
       ? 'bg-gradient-to-r from-forest to-sage text-mint'
       ...
       : 'bg-transparent text-ink-soft hover:text-ink hover:bg-mint/5'; ?>"
			...
			<!-- ============================================================
			     LOGIN PANEL
			     ============================================================ -->
			<div id="panel-login" class="tab-panel p-6 lg:p-8 <?php echo $active_tab === 'register' ? 'tab-panel--hidden' : ''; ?>">
...
?>
					...
					<div class="mb-5">
						...
						<div class="relative">
							<input
								class="woocommerce-Input woocommerce-Input--text input-text w-full px-4 py-3 pr-12 bg-white border border-ink-12 rounded-xl focus:border-sage focus:ring-2 focus:ring-sage/20 transition-all text-ink placeholder:text-ink-40"
								type="password"
								name="password"
								id="password"
							...
							<button
								...
								<span class="material-symbols-outlined text-xl" data-eye-icon="password">visibility</span>
							</button>
						</div>
					</div>
					...
					<div class="space-y-4 mb-5">
						<label class="flex items-center cursor-pointer group">
							<input
								class="w-4 h-4 accent-sage bg-white border border-ink-12 rounded focus:ring-2 focus:ring-sage/20 transition-all cursor-pointer"
								name="rememberme"
								type="checkbox"
								id="rememberme"
								value="forever" />
					...
					<p class="text-center text-sm text-ink-soft">
						...
						<button type="button" id="go-to-register" class="text-sage hover:text-ink font-semibold transition-colors ml-1">
			...
			<!-- ============================================================
			     REGISTER PANEL
			     ============================================================ -->
			<div id="panel-register" class="tab-panel p-6 lg:p-8 <?php echo $active_tab === 'login' ? 'tab-panel--hidden' : ''; ?>">

				<!-- Inline validation errors -->
				<div id="register-errors" class="hidden mb-5 p-4 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600 space-y-1"></div>
...
?>
					...
					<div class="space-y-3 mb-6">
						...
						<label class="flex items-start gap-3 cursor-pointer group">
							<input
								...
								id="reg_terms"
								class="mt-0.5 w-4 h-4 accent-sage bg-white border border-ink-12 rounded focus:ring-2 focus:ring-sage/20 transition-all cursor-pointer flex-shrink-0"
								required />
        ...
        ?>
					...
					<button
						type="submit"
						class="w-full inline-flex items-center justify-center gap-2 px-8 py-4 bg-gradient-to-r from-forest to-sage text-mint font-semibold rounded-xl hover:opacity-90 hover:scale-[1.01] active:scale-[0.99] transition-all shadow-sm hover:shadow-lg tracking-wide uppercase"
						name="register"
					...
					<p class="text-center text-sm text-ink-soft mt-4">
						...
						<button type="button" id="go-to-login" class="text-sage hover:text-ink font-semibold transition-colors ml-1">
...
(function () {
  ...
  function activateTab(tab) {
    ...
    var outPanel   = isRegister ? panelLogin : panelRegister;
    var inPanel    = isRegister ? panelRegister : panelLogin;

    // Animate out current panel
    outPanel.classList.add('tab-panel--animating-out');
    setTimeout(function () {
      outPanel.classList.add('tab-panel--hidden');
      outPanel.classList.remove('tab-panel--animating-out');

      // Prepare in-panel
      inPanel.classList.remove('tab-panel--hidden');
      inPanel.classList.add('tab-panel--animating-in');

      // Force reflow then animate in
      inPanel.offsetHeight; // eslint-disable-line no-unused-expressions
      inPanel.classList.remove('tab-panel--animating-in');
    }, 200);

    // Active tab styles
    var activeClasses   = ['bg-gradient-to-r', 'from-forest', 'to-sage', 'text-mint'];
  ...
  if (goToRegister) goToRegister.addEventListener('click', function () { activateTab('register'); });
  if (goToLogin)    goToLogin.addEventListener('click',    function () { activateTab('login'); });
  ...
  var strengthConfig = [
    { label: '<?php echo esc_js(__('Password strength: Too short', 'woocommerce')); ?>',   color: 'bg-red-400',    bars: 1 },
    { label: '<?php echo esc_js(__('Password strength: Weak', 'woocommerce')); ?>',        color: 'bg-orange-400', bars: 1 },
    { label: '<?php echo esc_js(__('Password strength: Fair', 'woocommerce')); ?>',        color: 'bg-yellow-400', bars: 2 },
    { label: '<?php echo esc_js(__('Password strength: Good', 'woocommerce')); ?>',        color: 'bg-blue-400',   bars: 3 },
    { label: '<?php echo esc_js(__('Password strength: Strong', 'woocommerce')); ?>',      color: 'bg-green-500',  bars: 4 },
  ];
  ...
  // ── Form submit guard (inline errors) ─────────────────────────────────────
  var registerForm  = document.getElementById('nera-register-form');
  var errorBox      = document.getElementById('register-errors');
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', function (e) {
      ...
      if (terms && !terms.checked) {
        errors.push('<?php echo esc_js(__('You must agree to the Terms & Conditions.', 'woocommerce')); ?>');
      }
      if (age && !age.checked) {
        errors.push('<?php echo esc_js(__('You must confirm you are over 18.', 'woocommerce')); ?>');
      }

      if (errors.length > 0) {
        e.preventDefault();
        errorBox.innerHTML = errors.map(function (msg) {
          return '<p class="flex items-center gap-2"><span class="material-symbols-outlined text-sm">error</span>' + msg + '</p>';
        }).join('');
