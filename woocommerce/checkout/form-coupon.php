...
<div class="bg-mint-wash rounded-2xl border border-ink-20 shadow-sm p-4 lg:p-6 mb-6" x-data="{ open: false }">
  ...
  <div x-show="open" 
       ...
       class="mt-4 pt-4 border-t border-ink-20 bg-ink-3 -mx-4 lg:-mx-6 px-4 lg:px-6 pb-4 rounded-b-xl">
        ...
        <div class="flex gap-2">
          <div class="flex-1 relative">
            ...
            <input type="text"
              name="coupon_code"
              id="coupon_code"
              class="w-full h-12 pl-4 pr-4 rounded-xl border-2 border-ink-20 bg-off-white text-ink text-sm font-medium focus:border-sage focus:ring-2 focus:ring-sage-20 outline-none transition-all placeholder:text-ink-40"
              placeholder="<?php esc_attr_e('Enter your discount code', 'nera-competitions'); ?>"
              value="" />
          </div>
          <button type="submit"
            class="h-12 px-6 rounded-xl bg-gradient-to-r from-forest to-sage text-white font-bold text-sm hover:opacity-90 active:scale-95 transition-all duration-200 shrink-0 flex items-center gap-2"
            name="apply_coupon"
    ...
    if (!empty($applied_coupons)): ?>
      <div class="mt-4 space-y-2" id="checkout-applied-coupons">
        <p class="text-xs font-semibold text-ink-56 uppercase tracking-wide mb-2"><?php esc_html_e(
          'Applied Coupons:',
          'nera-competitions',
        ); ?></p>
        <div class="flex flex-wrap gap-2">
          <?php foreach ($applied_coupons as $code): ?>
            <div class="inline-flex items-center gap-2 bg-sage-15 text-ink px-3 py-1.5 rounded-lg border border-ink-20 text-sm font-medium">
