...
foreach ($ticket_ids as $ticket_id):
  ...
  <tr class="hover:bg-mint-wash transition-colors">
    <?php foreach ($_columns as $key => $val): ?>
      <td class="px-4 py-3 text-ink text-sm" data-title="<?php echo esc_html($val); ?>">
        <?php
        switch ($key) {
          case 'ticket_number':
            echo '<span class="font-mono font-bold text-forest">' . esc_html($ticket->get_lottery_ticket_number()) . '</span>';
            break;
          case 'user_name':
