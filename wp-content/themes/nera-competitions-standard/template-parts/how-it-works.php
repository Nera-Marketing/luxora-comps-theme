...
// Get arguments with defaults from ACF if available, otherwise use $args or defaults
$title = get_field('hiw_hero_title') ?: (isset($args['title']) ? $args['title'] : __('How It Works', 'nera-competitions'));
...
// Default steps if not provided and not in ACF
$default_steps = [
  [
    'icon' =>
...
if (!empty($acf_steps) && is_array($acf_steps)) {
  $steps = [];
  foreach ($acf_steps as $i => $acf_step) {
    $default = isset($default_steps[$i]) ? $default_steps[$i] : $default_steps[0];
    $steps[] = [
      'icon' => !empty($acf_step['icon']) ? $acf_step['icon'] : $default['icon'],
      'title' => !empty($acf_step['title']) ? $acf_step['title'] : $default['title'],
      'description' => !empty($acf_step['description']) ? $acf_step['description'] : $default['description'],
      'color' => !empty($acf_step['color']) ? $acf_step['color'] : $default['color'],
      'bg_color' => !empty($acf_step['bg_color']) ? $acf_step['bg_color'] : $default['bg_color'],
    ];
  }
} else {
  $steps = isset($args['steps']) && is_array($args['steps']) ? $args['steps'] : $default_steps;
}
?>
