  ...
  <div class="bg-white rounded-2xl border border-border overflow-hidden">
    ...
    <div class="px-6 pb-6">
      <div class="flex items-center gap-2 mb-3">
        <?php if ($is_expired): ?>
          <span class="material-symbols-outlined text-ink-soft text-lg">lock</span>
          <span class="text-xs font-bold text-ink-soft uppercase tracking-wider">
            <?php _e('Competition Closed', 'nera-competitions'); ?>
          </span>
        <?php else: ?>
          <span class="material-symbols-outlined text-sage text-lg">schedule</span>
      ...
      <?php if (!empty($countdown_date_for_js) && !$is_expired): ?>
        ...
        <div class="lty-lottery-countdown-timer competition-countdown grid grid-cols-4 gap-3"
          ...
          <div class="text-center">
            ...
            <span class="block mt-2 text-xs font-semibold text-ink-soft uppercase tracking-wide">
      ...
      <?php elseif ($is_expired): ?>
        ...
        <div class="grid grid-cols-4 gap-3">
          <div class="text-center">
            <div class="bg-sage-40 rounded-xl p-3 lg:p-6">
      ...
      <?php else: ?>
        ...
        <div class="grid grid-cols-4 gap-3">
          <div class="text-center">
            <div class="bg-sage-dark rounded-xl p-3 lg:p-4">
    ...
    <div class="px-6 pb-6">
      ...
      <!-- Progress Bar -->
      <div class="h-2 w-full bg-ink-8 rounded-full overflow-hidden">
        <div class="h-full rounded-full transition-all duration-1000 ease-out" style="width: 0%; background: linear-gradient(90deg, var(--color-forest), var(--color-sage));"
          data-progress="<?php echo esc_attr($progress); ?>"></div>
      </div>

      <!-- Low Stock Warning -->
      <?php if ($is_low_stock): ?>
        <div class="mt-2 flex items-center gap-1 text-sage text-sm">
    ...
    <div class="px-6 pb-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <span
          class="inline-flex items-center gap-1 bg-sage-20 text-sage font-bold text-sm px-3 py-1 rounded-full">
          <?php echo wc_price($price); ?>
          <span class="font-normal text-sage-light-70">
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center border border-border rounded-lg overflow-hidden">
          <button type="button"
            class="w-10 h-10 flex items-center justify-center text-ink-soft hover:text-sage hover:bg-ink/5 transition-colors"
            data-quantity-minus>
          ...
          <input type="number" name="quantity" value="1"
            ...
            class="qty w-12 h-10 text-center font-semibold text-ink border-x border-border bg-off-white focus:outline-none"
            data-quantity-input />
          <button type="button"
            class="w-10 h-10 flex items-center justify-center text-ink-soft hover:text-sage hover:bg-ink/5 transition-colors"
            data-quantity-plus>
        ...
        <div class="flex items-center gap-2">
          ...
          <button type="button"
            class="px-4 h-10 border border-border text-ink-soft font-semibold text-sm rounded-lg hover:border-forest hover:text-sage transition-colors"
      ...
      <div x-data="purchaseCard({
        ...
        <form class="cart" @submit.prevent="submitForm"
          ...
          <?php if ($has_qa && $qa_can_display && !empty($questions)): ?>
            ...
            <div class="bg-white rounded-xl p-6 skill-challenge-wrapper border border-border">
              <div class="flex items-center gap-3 mb-4">
                <div class="w-8 h-8 rounded-full bg-sage-20 flex items-center justify-center text-sage">
                  <span class="material-symbols-outlined text-xl">psychology</span>
                </div>
                <h4 class="font-bold text-ink uppercase text-sm tracking-wide">
            ...
            $question = $questions[0]; ?>
              <div class="nera-contest-qa-container mb-5">
                <div class="nera-contest-question font-medium text-ink text-lg mb-4">
                ...
                <ul class="nera-contest-answers flex flex-col gap-3">
                  <?php foreach ($question['answers'] as $key => $answer): ?>
                    <li class="relative group cursor-pointer" @click="selectAnswer('<?php echo esc_js(
                      ...
                      <div class="flex items-center p-4 bg-off-white border-2 rounded-xl transition-all duration-200"
                        :class="selectedAnswer === '<?php echo esc_js(
                          $key,
                        ); ?>' ? 'border-sage ring-1 ring-sage-20 shadow-sm' : 'border-border hover:border-ink-50'">
                        <!-- Radio Circle -->
                        <div
                        ...
                        <span class="text-base font-medium"
                          :class="selectedAnswer === '<?php echo esc_js(
                            $key,
                          ); ?>' ? 'text-ink' : 'text-ink-soft group-hover:text-ink'">
            ...
            <?php elseif ($has_qa && !$qa_can_display): ?>
              ...
              <div class="border border-border rounded-xl p-5 mb-6 skill-challenge-wrapper">
                <div class="flex items-center gap-2 mb-4">
                  <span class="material-symbols-outlined text-sage text-lg">psychology</span>
                  <h4 class="font-bold text-ink uppercase text-sm tracking-wide">
                    <?php _e('Question Answers', 'nera-competitions'); ?>
                  </h4>
                </div>
            ...
            <button type="submit" name="add-to-cart" value="<?php echo esc_attr($product_id); ?>"
              class="w-full bg-forest text-mint text-lg py-4 rounded-xl font-bold shadow-lg hover:opacity-90 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed lty-participate-now-button"
    ...
    <div class="px-6 pb-6" x-data>
      <div class="border-t border-border">
        <p class="text-center text-sm text-ink-soft pt-6">
          <?php _e('Or', 'nera-competitions'); ?> 
          <button type="button" @click="$store.postDialog.show = true" class="text-sage font-semibold hover:text-ink transition-colors">
