...
<div class="blog-content-wrapper mb-16" data-aos="fade-up">
  <div class="prose prose-lg max-w-none 
                ...
                prose-p:text-ink-56 prose-p:font-body prose-p:leading-relaxed
                ...
                prose-li:text-ink-56
                selection:bg-sage/30">
    <?php the_content(); ?>
  </div>

  <?php if ($is_single): ?>
    <footer class="mt-16 pt-12 border-t border-ink-10 flex flex-wrap items-center justify-between gap-8">
      <!-- Author Card -->
      ...
      <?php if (has_tag()): ?>
        <div class="flex flex-wrap gap-2">
          ...
          $tags = get_the_tags();
          foreach ($tags as $tag) {
            echo '<a href="' . esc_url(get_tag_link($tag->term_id)) . '" class="px-4 py-2 rounded-full bg-white border border-ink-10 text-xs font-medium text-ink-56 hover:border-sage hover:text-sage transition-all">#' . esc_html($tag->name) . '</a>';
          }
          ?>
        </div>
      <?php endif; ?>
