...
class Table extends Tag {
	...
	public function open( $attr, &$ahtml, &$ihtml ) {
		...
		if ( $this->mpdf->tableLevel > 1 ) {
			$this->mpdf->table[ $this->mpdf->tableLevel ][ $this->mpdf->tbctr[ $this->mpdf->tableLevel ] ]['nestedpos'] = array(
				$this->mpdf->row,
				$this->mpdf->col,
				$this->mpdf->tbctr[ $this->mpdf->tableLevel - 1 ],
			);
		...
		if ( empty( $table['bgcolor'] ) && ! is_array( $table['bgcolor'] ) ) {
			$table['bgcolor'] = array();
		}
			...
			for ( $l = 0; $l < $this->mpdf->table[ $this->mpdf->tableLevel ][ $this->mpdf->tbctr[ $this->mpdf->tableLevel ] ]['nc']; $l++ ) {
				if ( ! isset( $this->mpdf->cell[ $k ][ $l ] ) ) {
					...
					if ( ! $this->mpdf->simpleTables ) {
						...
						$this->mpdf->cell[ $k ][ $l ]['border_details']['R']   = array(
							's'     => 0,
							'w'     => 0,
							'c'     => false,
							'style' => 'none',
						...
						$this->mpdf->cell[ $k ][ $l ]['border_details']['L']   = array(
							's'     => 0,
							'w'     => 0,
							'c'     => false,
							'style' => 'none',
						...
						$this->mpdf->cell[ $k ][ $l ]['border_details']['T']   = array(
							's'     => 0,
							'w'     => 0,
							'c'     => false,
							'style' => 'none',
						...
						$this->mpdf->cell[ $k ][ $l ]['border_details']['B']   = array(
							's'     => 0,
							'w'     => 0,
							'c'     => false,
							'style' => 'none',
						...
						$this->mpdf->cell[ $k ][ $l ]['border_details']['mbw'] = array(
							'BL' => 0,
							'BR' => 0,
							'RT' => 0,
							'RB' => 0,
