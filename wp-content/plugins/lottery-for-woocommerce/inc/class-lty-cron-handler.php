...
if ( ! class_exists( 'LTY_Cron_Handler' ) ) {
	...
	class LTY_Cron_Handler {
		...
		public static function get_wp_cron_schedules( $type = 'update' ) {
			...
			switch ( $type ) {
				...
				case 'relist':
					...
					$cron_type     = isset( $cron_settings['unit'] ) ? $cron_settings['unit'] : 'hours';
					$cron_time     = isset( $cron_settings['number'] ) ? intval( $cron_settings['number'] ) : 5;
					break;
			}
		 ...
		 * @since 7.0
		 * */
		public static function schedule_lottery_started_emails( $product_id ) {
			$product = wc_get_product( $product_id );
			...
			if ( 'yes' !== get_option( 'lty_customer_lottery_started_enabled' ) ) {
				return;
			}
