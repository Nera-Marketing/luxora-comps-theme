...
?>
<div class="lty-lottery-question-answer-container"
	data-force="<?php echo esc_attr($product->is_force_answer_enabled()); ?>">
	<h3>
		<?php echo wp_kses_post(get_option('lty_settings_single_product_question_answer_heading_label', __('Question Answers', 'lottery-for-woocommerce'))); ?>
	</h3>
	...
	if ('1' === $product->get_question_answer_display_type()):
		...
		<ul class="lty-lottery-answers">
			...
			foreach ($question['answers'] as $answer_id => $answer):
				...
				?>
				<li class="<?php echo esc_attr($class_name); ?>" data-answer-id="<?php echo esc_attr($answer_id); ?>">
					<?php echo esc_html($answer['label']); ?>
				</li>
			<?php endforeach; ?>
			<input type="hidden" class="lty-question-answer-id" name="lty_question_answer_id"
				value="<?php echo esc_html($cart_answer_id); ?>" />
		</ul>
		<?php
	else:
		?>
		<p class='lty-lottery-options-field-wrapper'>
			<select id='lty_lottery_options_field' class='lty-lottery-answers lty-question-answer-id'
				name="lty_question_answer_id">
				...
				foreach ($question['answers'] as $answer_id => $answer):
					$class_name = $cart_answer_id == $answer_id ? 'lty-selected' : '';
					?>
					<option value="<?php echo esc_attr($answer_id); ?>" class="<?php echo esc_attr($class_name); ?>" <?php echo selected('lty-selected', $class_name); ?> data-answer-id="<?php echo esc_attr($answer_id); ?>">
						<?php echo esc_html($answer['label']); ?>
					</option>
					<?php
				endforeach;
