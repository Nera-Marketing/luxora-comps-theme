...
<div class='lty-lottery-range-slider-wrapper'>
	<div class='lty-lottery-range-slider-content'>
		...
		<div class='lty-quantity-range-slider' data-min='<?php echo esc_attr($min_value); ?>'
			data-max='<?php echo esc_attr($max_value); ?>' data-preset='<?php echo esc_attr($preset_value); ?>'
			...
			if ($product->is_predefined_button_enabled() && $product->can_display_range_slider_predefined_buttons_discount_tag() && !empty($product->get_lty_range_slider_predefined_discount_label())):
				foreach ($product->get_predefined_buttons_rule() as $predefined_button_id => $button_data):
					...
					?>
					<span class="lty-range-slider-discount lty-range-slider-discount-<?php echo esc_attr($ticket_quantity); ?>"
						style="left:calc( <?php echo esc_attr($percentage); ?>% - 30px);"><?php echo wp_kses_post($product->get_range_slider_predefined_discount_label($predefined_button_id, $ticket_quantity)); ?></span>
					<?php
				endforeach;
			endif;
			?>
			<span
				class='lty-lottery-range-value'><?php echo wp_kses_post(lty_get_lottery_range_slider_message($min_value)); ?></span>
		</div>

		<input type='button' value='+' class='lty-lottery-range-slider-increment lty-lottery-range-slider-btn'>
		<input type='hidden'
			class="lty-quantity-selector lty-product-quantity-<?php echo esc_attr($product->get_id()); ?>" name='quantity'
