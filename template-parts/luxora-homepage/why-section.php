...
<section class="bg-forest text-white py-12 px-4 sm:py-16 sm:px-6 md:py-20 md:px-10 lg:py-24 lg:px-[60px] grid grid-cols-1 gap-10 md:gap-16 lg:grid-cols-2 lg:gap-20 items-start lg:items-center why-section">
  <div class="relative aspect-[3/4] overflow-hidden w-full max-w-lg mx-auto lg:max-w-none lg:mx-0 why-visual">
    ...
    <?php if ($pullquote): ?>
      <div class="absolute bottom-[34%] left-0 bg-mint p-4 sm:p-[18px_20px] max-w-[min(195px,calc(100%-1rem))] sm:max-w-[195px] why-pullquote">
  ...
  <div>
    ...
    <h2 class="font-heading text-[clamp(1.9rem,3vw,2.7rem)] font-normal text-white leading-[1.15] mb-6 md:mb-8 lg:mb-10 why-title"><?php echo esc_html($title); ?><br><em class="italic text-mint"><?php echo esc_html($title_em); ?></em></h2>
    <?php if (is_array($items) && !empty($items)): ?>
      <div class="flex flex-col why-items">
        <?php foreach (array_slice($items, 0, 4) as $n => $item): ?>
          <div class="flex gap-[22px] py-5 sm:py-[26px] border-b border-[rgba(200,230,192,0.08)] first:pt-0 why-item">
