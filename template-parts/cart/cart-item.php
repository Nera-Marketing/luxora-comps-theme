...
<div class="nera-cart-item relative"
  ...
  <div class="cart-item-mobile flex flex-col gap-3 py-3 md:hidden">
    <div class="flex gap-3 items-center">
      <a href="<?php echo esc_url($product->get_permalink()); ?>"
        class="w-16 h-16 rounded-lg overflow-hidden bg-off-white flex-shrink-0 relative group block">
        <?php echo $thumbnail; ?>
        <span class="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity"></span>
      </a>
      <div class="flex-1 min-w-0">
        <h3 class="font-semibold text-base text-ink leading-tight truncate">
        ...
        <div class="flex items-center gap-2 text-xs text-text-secondary flex-wrap mt-0.5">
          ...
          <?php endif; ?>
          <?php if ($answers): ?>
            <span class="text-ink-soft"><?php echo esc_html__(
              'Answer:',
              'nera-competitions',
  ...
  $max_per_order,
          ...
          <div class="nera-cart-quantity h-9">
            <button type="button" class="w-7 h-full flex items-center justify-center text-ink-soft hover:text-sage hover:bg-ink/5 transition-colors" @click="if(qty > min) { qty--; }">
  ...
  $max_per_order,
); ?>" class="w-10 h-full text-center border-x border-border text-sm font-semibold text-ink bg-off-white focus:outline-none qty" x-model="qty">
            <button type="button" class="w-7 h-full flex items-center justify-center text-ink-soft hover:text-sage hover:bg-ink/5 transition-colors" @click="if(qty < max) { qty++; }">
      ...
      <?php endif; ?>
      <span class="text-base font-bold text-ink whitespace-nowrap"><?php echo WC()->cart->get_product_subtotal(
        $product,
        $quantity,
      ); ?></span>
  ...
  <div class="cart-item-desktop hidden md:flex flex-col md:flex-row gap-4 md:items-center">
    ...
    <div class="flex-grow">
      <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-2">
        <div>
          <h3 class="font-bold text-lg text-ink leading-tight mb-1">
            <a href="<?php echo esc_url(
              $product->get_permalink(),
            ); ?>" class="hover:text-sage transition-colors">
              <?php echo wp_kses_post($product_name); ?>
          ...
          <?php if ($answers): ?>
            <div class="text-xs text-ink-soft mt-1"><span class="font-medium">Selected Answer:</span> <?php echo esc_html(
              $answers,
            ); ?></div>
          <?php endif; ?>
        </div>
  ...
  $max_per_order,
            ...
            <div class="nera-cart-quantity h-10">
              <button type="button" class="w-8 h-full flex items-center justify-center text-ink-soft hover:text-sage hover:bg-ink/5 transition-colors" @click="if(qty > min) { qty--; }">
                <span class="material-symbols-outlined text-sm">remove</span>
              </button>
              <input type="number" name="cart[<?php echo esc_attr(
                $cart_item_key,
  ...
  $max_per_order,
); ?>" class="w-12 h-full text-center border-x border-border text-sm font-semibold text-ink bg-off-white focus:outline-none qty" x-model="qty">
              <button type="button" class="w-8 h-full flex items-center justify-center text-ink-soft hover:text-sage hover:bg-ink/5 transition-colors" @click="if(qty < max) { qty++; }">
                <span class="material-symbols-outlined text-sm">add</span>
              </button>
            </div>
            <div class="flex items-center gap-1">
              <button type="button" class="px-2 py-1 text-xs font-semibold text-sage bg-sage/20 hover:bg-sage/30 rounded transition-colors" @click="let newQty = Math.min(qty + 5, max); qty = newQty;">+5</button>
              <button type="button" class="px-2 py-1 text-xs font-semibold text-sage bg-sage/20 hover:bg-sage/30 rounded transition-colors" @click="let newQty = Math.min(qty + 10, max); qty = newQty;">+10</button>
            </div>
          </div>
        <?php else: ?>
          <div class="flex items-center gap-3 w-full md:w-auto">
            <div class="inline-flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-lg">
              ...
              <span class="text-sm font-bold text-ink"><?php echo esc_html(
                $quantity,
              ); ?></span>
            </div>
            <input type="hidden" name="cart[<?php echo esc_attr(
        ...
        <div class="text-right w-full md:w-auto">
          <span class="block text-xs text-ink-soft md:hidden mb-1"><?php _e(
            'Total',
            'nera-competitions',
          ); ?></span>
          <span class="text-xl font-bold text-ink"><?php echo WC()->cart->get_product_subtotal(
            $product,
            $quantity,
          ); ?></span>
        </div>
