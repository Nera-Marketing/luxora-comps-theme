...
<div class="bg-white rounded-2xl border border-border shadow-sm p-6">
  ...
  <h2 class="text-xl font-bold text-ink mb-6 flex items-center gap-2">
    <span class="material-symbols-outlined text-sage">credit_card</span>
    ...
    <div class="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
      ...
      if ($terms_page_id > 0 && apply_filters('woocommerce_checkout_show_terms', true)): ?>
        <div class="woocommerce-terms-and-conditions-wrapper">
          <label class="flex items-start gap-3 cursor-pointer text-sm leading-relaxed text-ink-56">
            <input type="checkbox" name="terms" id="terms"
              class="mt-0.5 h-5 w-5 shrink-0 cursor-pointer rounded border-ink-30 accent-sage focus:ring-sage-30 focus:ring-2 focus:ring-offset-2">
            <span>
              <?php printf(
                wp_kses_post(__('I have read and agree to the website %s', 'woocommerce')),
                '<a href="' .
                  esc_url(get_permalink($terms_page_id)) .
                  '" class="font-semibold text-sage underline underline-offset-2 transition-colors hover:text-ink" target="_blank">' .
